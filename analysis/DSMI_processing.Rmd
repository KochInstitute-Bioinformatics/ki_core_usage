...
bmc.ext.dsmi <- bmc.ext.dsmi %>% dplyr::mutate(Charge.ID = "")
```
...
### Add hours for Anderson Scott's work

Import the data exported from PMIT

```{r}
as.hours <- read.xlsx("AS_hours_toDSMI.xlsx")
```

Make repairs prior to merge

```{r}
as.hours <- as.hours %>% rename(IGT_DSMI_Sponsor = PMIT_Sponsor)
as.hours <- as.hours %>% dplyr::select(-Charge.Name.Short)
```

Add Anderson's hours to the hours.project

```{r}
hours.project <- rbind(hours.project,as.hours)
```
