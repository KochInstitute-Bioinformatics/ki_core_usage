...
```{r}
train.1 <- igt.final %>% dplyr::filter(str_detect(Usage.Type, "Assisted|Training|Unattended"))
cat("Rows train1:", dim(train.1)[1], "\n")
train.2 <- igt.final %>% dplyr::filter(Usage.Type == "N/A" & Category == "SEM/FIB/Airyscan charges")
cat("Rows train2:", dim(train.2)[1], "\n")
train.3 <- igt.final %>% dplyr::filter(Usage.Type == "N/A" & Category == "Technical Time" & str_detect(Charge.Name, "Training"))
cat("Rows train3:", dim(train.3)[1], "\n")
train.4 <- igt.final %>% dplyr::filter(Usage.Type == "N/A" & 
...
train.pie <- train %>%
  ...
  summarise(Quantity = sum(Quantity), 
            Num_Labs = if_else(first(Sponsor_ForPlot) == "Center_Member",
                               n_distinct(Faculty.Key[DSMI_IGT_Sponsor == "KI_member"]),
