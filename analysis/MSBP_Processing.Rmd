...
metpro <- metpro %>% dplyr::mutate(Charge.ID = "")
```
...
synth.pie <- pep.synth %>%
  ...
  labs(title = "Peptide Synthesis (syntheses)",
       fill = "Sponsor") +
  theme_void() +
...
lip.met.pie <- lip.met %>%
  ...
  geom_text_repel(aes(label = Label,
                      vjust = case_when(Sponsor_ForPlot == "Center_Member" ~ 3.5,
                                        TRUE ~ 0.5),
                      segment.size = case_when(Sponsor_ForPlot == "Center_Member" ~ 0,
                                               TRUE ~ 0.5)),
                  size = 3.6,
                  fontface = "bold",
                  color = "black",
  ...
  coord_polar("y", start = 0) +
  scale_fill_manual(values = sponsor_colors) +
  labs(title = "Lipid/Metabolite Profiling (instrument runs)",
       fill = "Sponsor") +
  theme_void() +
  theme(legend.position = "bottom",
        plot.title = element_text(size = 12,
...
time.pie <- time %>%
  mutate(Sponsor_ForPlot = factor(Sponsor_ForPlot, levels = sponsor_order)) %>%
  group_by(Sponsor_ForPlot) %>%
  summarise(Quantity = sum(round(Quant.To.Present,0)), 
            Num_Labs = if_else(first(Sponsor_ForPlot) == "Center_Member",
                               n_distinct(Faculty.Key[Sponsor == "KI_member"]),
                               n_distinct(Faculty.Key)),
            .groups = "drop") %>%
...
synth.pie + prot.pie + lip.met.pie + maldi.pie + time.pie + plot_layout(ncol = 3, guides = "collect") & 
  theme(legend.position = "bottom")

combo.pie <- synth.pie + prot.pie + lip.met.pie + maldi.pie + time.pie + plot_layout(ncol = 3, guides = "collect") & 
  theme(legend.position = "bottom")
