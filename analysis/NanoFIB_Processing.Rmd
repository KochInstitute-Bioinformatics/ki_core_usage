...
rate_modifiers <- tibble(
  NanoFIB_Sponsor = c("KI_member", "KI_affiliate", "MIT_sponsor", "MIT_non_sponsor", "external", "industry"),
  RateModifier = c(1, 1, 1, 0.5, 0.5, 0.167)
)
```
...
```{r}
table(ilabs.nano$NanoFIB_Sponsor,ilabs.nano$RateModifier)
```

Calculate adjusted price column for pie charts

```{r}
ilabs.nano <- ilabs.nano %>% mutate(Adjusted.Total.Price = Total.Price * RateModifier)
```
...
sample.pie <- sample %>%
  ...
  coord_polar("y", start = 0) +
  scale_fill_manual(values = sponsor_colors) +
  labs(title = "EM Sample Preparation\n(relative dollars)",
       fill = "Sponsor") +
  theme_void() +
  theme(legend.position = "bottom",
        plot.title = element_text(size = 12,
